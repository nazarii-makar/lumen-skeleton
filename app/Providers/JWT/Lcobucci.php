...
class Lcobucci extends Provider implements JWT
{
    ...
    public function decode($token)
    {
        ...
        if ( ! $jwt->hasClaim('iss')) {
            throw new TokenInvalidException('Invalid value provided for claim [iss]');
        }

        if ( ! $jwt->verify($this->signer, $this->getIssuerVerificationKey($jwt->getClaim('iss')))) {
            throw new TokenInvalidException('Token Signature could not be verified.');
        }

        return (new Collection($jwt->getClaims()))->map(function ($claim) {
            /** @var Claim|mixed $claim */
    ...
    /**
     * @return \Lcobucci\JWT\Signer\Key|resource|string
     */
     ...
     * @return mixed
     */
    public function getIssuerPublicKey($issuer)
    {
        return config('jwt.issuer_keys')[$issuer]['public'] ?? $this->getPublicKey();
    }
     ...
     * @return mixed
     */
    public function getIssuerSecret($issuer)
    {
        return config('jwt.issuer_keys')[$issuer]['secret'] ?? $this->getSecret();
    }
     ...
     */
    protected function getIssuerVerificationKey($issuer)
    {
        return $this->isAsymmetric() ?
            (new Keychain())->getPublicKey($this->getIssuerPublicKey($issuer)) :
            $this->getIssuerSecret($issuer);
    }
